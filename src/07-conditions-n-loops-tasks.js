...
function doRectanglesOverlap(rect1, rect2) {
  if (Math.abs(rect1.top - rect2.top) > Math.max(rect1.height, rect2.height)) return false;
  if (Math.abs(rect1.left - rect2.left) > Math.max(rect1.width, rect2.width)) return false;
  return true;
}
...
function isInsideCircle(circle, point) {
  const { center } = circle;
  const hypothenuse = Math.sqrt((center.x - point.x) ** 2 + (center.y - point.y) ** 2);
  if (hypothenuse < circle.radius) return true;
  return false;
}
...
function findFirstSingleChar(str) {
  const res = str.split('').filter((char) => Array.from(str.matchAll(char)).length === 1);
...
function getDigitalRoot(num) {
  if (num <= 9) return num;
  return getDigitalRoot(num.toString().split('').reduce((s, n) => s + +n, 0));
}
...
function getCommonDirectoryPath(pathes) {
  const map = pathes.map((str) => str.split('/'))
...
function getMatrixProduct(m1, m2) {
  return m1.map((row) => {
    ...
    for (let i = 0; i < m2[0].length; i += 1) {
      res.push(row.map((el, inx) => el * m2[inx][i]).reduce((sum, x) => sum + x, 0));
    }
    return res;
  });
}
